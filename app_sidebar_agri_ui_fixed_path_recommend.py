...
# 说明：
# 1. 系统启动后默认使用“性价比高”组合；
# 2. 这里使用固定路径，不在启动阶段递归搜索项目文件夹，因此页面加载更快；
# 3. 若你的模型实际路径不同，只需要修改下面四个路径；
# 4. 其他功能模块保持不变。
DEFAULT_MODEL_PRESET = "性价比高"

FIXED_MODEL_PRESETS = {
    "精准度高": {
        "det": r"D:\pyhon\xiangmu\detect_compare_runs\yolov8s\weights\best.pt",
        "cls": r"D:\pyhon\newA_vscode\wheat_drought_runs\exp_augmented2_s\weights\best.onnx",
        "desc": "YOLOv8s 检测模型 + YOLOv8s-cls 分类模型，整体精度较高，适合实验展示和结果分析。",
    },
    "性价比高": {
        "det": r"D:\pyhon\xiangmu\detect_compare_runs\yolov8n\weights\best.pt",
        "cls": r"D:\pyhon\newA_vscode\wheat_drought_runs\exp_augmented2\weights\best.onnx",
        "desc": "YOLOv8n 检测模型 + YOLOv8n-cls 分类模型，速度较快、模型较轻，适合作为系统默认加载组合。",
    },
}
...
def get_fixed_preset_paths(preset_name: str) -> Tuple[str, str]:
    """返回固定路径推荐搭配。"""
    cfg = FIXED_MODEL_PRESETS.get(preset_name, FIXED_MODEL_PRESETS[DEFAULT_MODEL_PRESET])
    return cfg["det"], cfg["cls"]


def default_detector_path() -> str:
    """进入系统后默认加载“性价比高”检测模型固定路径。"""
    det_path, _ = get_fixed_preset_paths(DEFAULT_MODEL_PRESET)
    return det_path


def default_classifier_path() -> str:
    """进入系统后默认加载“性价比高”分类模型固定路径。"""
    _, cls_path = get_fixed_preset_paths(DEFAULT_MODEL_PRESET)
    return cls_path


def apply_fixed_model_preset(preset_name: str) -> None:
    """应用固定路径推荐模型搭配，并清理缓存。"""
    det_path, cls_path = get_fixed_preset_paths(preset_name)

    if not Path(det_path).is_file():
        st.warning(f"检测模型路径不存在，请检查：{det_path}")
        return

    if not Path(cls_path).is_file():
        st.warning(f"分类模型路径不存在，请检查：{cls_path}")
        return

    apply_model_config(det_path, cls_path)
    st.session_state.model_preset = preset_name
    st.success(f"已应用推荐搭配：{preset_name}")
...
def ensure_session() -> None:
    ...
    if "model_preset" not in st.session_state:
        st.session_state.model_preset = DEFAULT_MODEL_PRESET
...
def render_models_page() -> None:
    ...
    # 推荐模型搭配：固定路径版，不进行启动时递归搜索
    section("推荐搭配")
    preset_names = list(FIXED_MODEL_PRESETS.keys())
    default_idx = preset_names.index(st.session_state.model_preset) if st.session_state.model_preset in preset_names else preset_names.index(DEFAULT_MODEL_PRESET)
    ...
    selected_preset = st.radio(
        ...
        index=default_idx,
        horizontal=True,
        help="精准度高适合实验展示；性价比高适合作为系统默认加载组合。"
    )

    det_rec, cls_rec = get_fixed_preset_paths(selected_preset)
    st.info(FIXED_MODEL_PRESETS[selected_preset]["desc"])

    rc1, rc2, rc3 = st.columns([1, 1, 0.55], gap="large")
    with rc1:
        mini_card("推荐检测模型", det_rec)
        if Path(det_rec).is_file():
            st.success("检测模型路径有效")
        else:
            st.warning("检测模型路径不存在")
    with rc2:
        mini_card("推荐分类模型", cls_rec)
        if Path(cls_rec).is_file():
            st.success("分类模型路径有效")
        else:
            st.warning("分类模型路径不存在")
    with rc3:
        if st.button("应用推荐搭配", type="primary"):
            apply_fixed_model_preset(selected_preset)
    section_end()
